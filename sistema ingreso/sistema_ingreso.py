from crear_funciones import crear_contrasenia_random
import random
print('Bienvenido al sistema de matricula')

base_datos={'Nanu2629':'hola234',
           'Sofybc':'clirbi'}
num = random.choice([4,3,2,5,7,12,9,1])
...
def registro():
    ...
    if usuario in base_datos:
        ...
        contra_prueb = crear_contrasenia_random(num)
        print(f'Contraseña generada por el sistema como ayuda: {contra_prueb}')
        contraseña = input('Ingrese una contraseña: ')
        base_datos[usuario] = contraseña
        '''  with open('base_datos.txt','w')as archivo:
                [archivo.writelines(f'{usuario}:{contraseña}')for usuario,contraseña in (base_datos)]'''
